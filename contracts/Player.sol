...
contract Player is ERC721URIStorage {
    ...
    function signUp()
        ...
        returns (uint256)
    {
        require(balanceOf(msg.sender) == 0, "Address already registered");
        _tokenIds.increment();
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
     ...
     function tokenURITemplate(address tokenOwner) private view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Player: ', parts[2], '", "description": "Player NFT which is stored on chain grants access to the OSU game.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
